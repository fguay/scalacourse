...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    (for{
      old <- b.legalNeighbors
    } yield (old._1, old._2 :: history)).toStream
  }
   ...
   */
  lazy val pathsFromStart: Stream[(Block, List[Move])] = from((startBlock, List()) #:: Stream.empty, (startBlock :: Nil).toSet)
