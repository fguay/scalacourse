  ...
  def descendingByRetweet: TweetList = {	  
	  if(isEmpty) {
	    Nil
	      ...
	      val mostTweet: Tweet = mostRetweeted
...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new IndexOutOfBoundsException("No tweet available.");
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = left.union(right.union(that)).incl(elem)
   ...
   def mostRetweeted: Tweet = {
	    def max(max: Tweet, t: TweetSet): Tweet = {
	      if(t.isEmpty) max 
	      else{
	        val otherT = t.mostRetweeted;
	        if(max.retweets > otherT.retweets) max
	        else otherT
	      }
	    }
